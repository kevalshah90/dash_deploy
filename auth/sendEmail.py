...
from .models import pass_recovery, email_verification
from . import db
import random
...
def createTokenForRecovery(email):
    ...
    passrecovery = pass_recovery(email=email, token=random_string, status=0,created=datetime.now())

    # add the new user to the database
    db.session.add(passrecovery)
    db.session.commit()
...
def createTokenForEmailVerification(email):
    ...
    pass_verification = email_verification(email=email, token=random_string, status=0, created=datetime.now())
